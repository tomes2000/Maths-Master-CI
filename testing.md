...
| 3 | Quiz wouldn't work when easy and hard mode were selected. However, strangely medium difficult would work perfectly. This was really bad as it meant that the user was restricted to playing the quiz on medium difficulty only| After further research and troubleshooting, I found out that the problem was with the API. Unfortunately, there was a bug within the API that meant that the specific topic that I wanted didn't populate any questions for the quiz to utilise in the easy and hard difficulty settings, so I just changed the topic and it works. |
